...
const segmentStyles: Record<string, { label: string; class: string }> = {
  ...
  returning: { label: "ขาประจำ", class: "bg-teal-500/10 text-teal-400" },
  vip: { label: "VIP", class: "bg-gradient-to-r from-amber-500/10 to-amber-500/10 text-amber-400 border-amber-500/20" },
  ...
  return (
    <AppLayout>
      <div className="space-y-6">
        <div>
          <h1 className="text-xl font-bold bg-gradient-to-r from-teal-300 to-cyan-300 bg-clip-text text-transparent">ลูกค้า</h1>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <Input data-testid="search-customers" placeholder="ค้นหาลูกค้า..." className="pl-9 bg-white/[0.04] border-white/[0.06] focus:border-teal-500/40 text-white/80 placeholder:text-white/30" value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <div className="flex gap-2">
            {[{ key: "all", label: "ทั้งหมด" }, ...Object.entries(segmentStyles).map(([k, v]) => ({ key: k, label: v.label }))].map(s => (
              <button key={s.key} data-testid={`filter-seg-${s.key}`} onClick={() => setSegFilter(s.key)}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 ${segFilter === s.key ? "bg-gradient-to-r from-teal-500 to-cyan-600 text-white shadow-lg shadow-teal-500/20" : "bg-white/[0.04] text-white/50 hover:bg-white/[0.06] hover:text-white/70"}`}>
                {s.label}
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(customer => {
            ...
            return (
              <div key={customer.id} className="bg-white/[0.02] border border-white/[0.06] rounded-2xl hover:border-teal-500/20 hover:bg-white/[0.04] transition-all duration-300 overflow-hidden">
                <div className="p-4">
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-full bg-gradient-to-r from-teal-500/30 to-cyan-500/30 flex items-center justify-center text-teal-300 font-bold text-sm shrink-0 ring-1 ring-teal-500/20">
                  ...
                  <div className="grid grid-cols-2 gap-3 mt-3 pt-3 border-t border-white/[0.06]">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-teal-400">฿{customer.totalSpent.toLocaleString()}</p>
                    </div>
                  </div>
                </div>
              </div>
