...
const features = [
  { icon: Bot, title: "AI Agent ที่มีความจำ", desc: "AI ที่จำบริบทธุรกิจ วิเคราะห์ข้อมูล และเรียนรู้จากพฤติกรรมลูกค้าตลอดเวลา", color: "from-teal-500 to-cyan-500" },
  { icon: Sparkles, title: "Generative UI", desc: "สร้างหน้าร้าน ภาพสินค้า และคอนเทนต์อัตโนมัติด้วย AI", color: "from-violet-500 to-fuchsia-500" },
  { icon: TrendingUp, title: "Dynamic Pricing", desc: "ปรับราคาอัจฉริยะตามอุปสงค์และราคาคู่แข่งแบบ Real-time", color: "from-amber-500 to-yellow-500" },
  { icon: ShoppingBag, title: "Shopping Assistant", desc: "ผู้ช่วยช้อปปิ้งส่วนตัวที่เข้าใจภาษาธรรมชาติ แนะนำสินค้าตรงใจ", color: "from-emerald-500 to-teal-500" },
  { icon: Eye, title: "Visual Search", desc: "ค้นหาสินค้าด้วยรูปภาพ ใช้ Computer Vision ขั้นสูง", color: "from-pink-500 to-rose-500" },
  { icon: BarChart3, title: "AI Analytics", desc: "วิเคราะห์ยอดขาย พยากรณ์เทรนด์ และแนะนำกลยุทธ์ด้วย AI", color: "from-sky-500 to-blue-500" },
];
  ...
  return (
    <div ref={ref} className="relative p-5 rounded-2xl border border-border/50 bg-card/50 backdrop-blur-sm group hover:border-primary/40 transition-all duration-300 card-glow overflow-hidden">
      ...
      <div className="relative">
        <div className={`text-2xl font-black bg-gradient-to-r ${color} bg-clip-text text-transparent animate-count-up`}>
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <nav className="sticky top-0 z-50 border-b border-border/50 bg-background/80 backdrop-blur-lg">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/auth"><Button data-testid="nav-signup" className="bg-gradient-to-r from-teal-500 to-cyan-600 text-white text-sm hover:from-teal-600 hover:to-cyan-700 shadow-lg shadow-teal-500/20">เปิดร้านฟรี</Button></Link>
          </div>
        </div>
      </nav>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-fuchsia-500/5 rounded-full blur-[120px] animate-glow-pulse" style={{ animationDelay: '0.8s' }} />
        ...
        <div className="max-w-4xl mx-auto px-4 py-20 sm:py-28 text-center relative z-10">
          <Badge variant="outline" className="mb-5 text-xs px-4 py-1.5 border-emerald-500/30 bg-emerald-500/5 animate-slide-up">
          ...
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black leading-tight animate-slide-up" style={{ animationDelay: '100ms' }}>
            ...
            <span className="bg-gradient-to-r from-teal-400 via-cyan-300 to-sky-400 bg-clip-text text-transparent animate-gradient-shift">ด้วย AI ที่ชาญฉลาดที่สุด</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mt-8 animate-slide-up" style={{ animationDelay: '300ms' }}>
            <Link href="/auth">
              <Button data-testid="hero-cta" size="lg" className="bg-gradient-to-r from-teal-500 to-cyan-600 text-white px-8 shadow-lg shadow-teal-500/25 hover:shadow-teal-500/40 hover:scale-105 transition-all duration-300">
                <Flame className="w-4 h-4 mr-1" /> เปิดร้านฟรี <ArrowRight className="w-4 h-4 ml-1" />
              </Button>
            </Link>
            <Link href="/mall">
              <Button variant="outline" size="lg" className="border-teal-500/30 hover:bg-teal-500/5 hover:border-teal-500/50"><ShoppingBag className="w-4 h-4 mr-1" /> Shopping Mall</Button>
            </Link>
          </div>
      ...
      <section className="max-w-5xl mx-auto px-4 py-16">
        <div className="text-center mb-12">
          <Badge variant="outline" className="mb-3 border-teal-500/30 bg-teal-500/5">
            <Rocket className="w-3 h-3 mr-1 text-teal-500" />ง่ายมาก
          </Badge>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 stagger-children">
          {howItWorks.map((item, i) => (
            <Card key={item.step} className="border-border/50 text-center group hover:border-primary/30 transition-all duration-300 card-glow animate-slide-up overflow-hidden relative">
              <div className="absolute top-0 inset-x-0 h-1 bg-gradient-to-r from-teal-400 via-cyan-400 to-sky-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              <CardContent className="p-6">
                <div className="w-14 h-14 mx-auto rounded-2xl bg-gradient-to-br from-teal-500/10 to-cyan-500/10 flex items-center justify-center mb-4 group-hover:scale-110 transition-transform duration-300">
                  <item.icon className="w-7 h-7 text-teal-500" />
                </div>
                <div className="inline-flex items-center justify-center w-7 h-7 rounded-full bg-gradient-to-r from-teal-500 to-cyan-600 text-white text-xs font-bold mb-2">{item.step}</div>
      ...
      <section className="max-w-6xl mx-auto px-4 py-16">
        <div className="text-center mb-12">
          <Badge variant="outline" className="mb-3 border-violet-500/30 bg-violet-500/5">
            <Zap className="w-3 h-3 mr-1 text-teal-500" />ฟีเจอร์ที่ทรงพลัง
          </Badge>
      ...
      <section className="max-w-5xl mx-auto px-4 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5 stagger-children">
          {[
            ...
            <Card key={p.name} className={`border-border/50 animate-slide-up overflow-hidden relative transition-all duration-300 ${p.highlight ? "ring-2 ring-teal-500/60 shadow-xl shadow-teal-500/10" : "card-glow"}`}>
              {p.highlight && (
                <>
              ...
              <CardContent className="p-6">
                ...
                <Link href="/auth">
                  <Button className={`w-full mt-6 ${p.highlight ? "bg-gradient-to-r from-teal-500 to-cyan-600 text-white shadow-lg shadow-teal-500/20 hover:shadow-teal-500/40" : "hover:border-teal-500/30"}`} variant={p.highlight ? "default" : "outline"}>
      ...
      <section className="max-w-3xl mx-auto px-4 py-16 text-center relative">
        ...
        <div className="relative">
          ...
          <Link href="/auth">
            <Button size="lg" className="mt-6 bg-gradient-to-r from-teal-500 to-cyan-600 text-white px-8 shadow-lg shadow-teal-500/25 hover:shadow-teal-500/40 hover:scale-105 transition-all duration-300">
      ...
      <footer className="border-t border-border/50 py-8">
        <div className="max-w-6xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-muted-foreground">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded bg-gradient-to-br from-teal-500 to-cyan-600 flex items-center justify-center text-white font-bold text-[10px]">Z</div>
